...
#include "edge_tts/serialization/MetadataJsonParser.hpp"
...
parse_binary_frame(const std::vector<std::byte>& data)
{
    ...
    // Parse header content (bytes 2..header_length) using ProtocolParser.
    // Append \r\n\r\n so ProtocolParser finds the required separator.
    const std::size_t header_content_len = (header_length >= 2) ? (header_length - 2) : 0;
    ...
    // Body starts at header_length + 2 (skip separator).
    // Safe: if header_length + 2 > data.size(), subspan is empty.
    const std::size_t body_offset =
        (header_length + 2 <= data.size()) ? (header_length + 2) : data.size();
